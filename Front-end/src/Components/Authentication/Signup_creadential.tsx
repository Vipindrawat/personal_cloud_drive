...
const Signup_creadential = (props: typeofprops) => {
  ...
  const [signup_cred, setsignup_cred] = useState<{ signup_name: string, signup_password: string, signup_cpassword: string }>({ signup_name: "", signup_password: "", signup_cpassword: "" });

  const [signup_validation, setsignup_validation] = useState<{ name_validation: string, password_validation: string, cpassword_validation: string }>({ name_validation: "", password_validation: "", cpassword_validation: "" });
  ...
  const signup_submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const user_register = await fetch(`${host}`, {
      ...
      body: JSON.stringify({ email, otp: otp.join(''), password: signup_cred.signup_password, name: signup_cred.signup_name })
    })
    ...
    if (json.success == true) {
      ...
      setsignup_cred({ signup_name: "", signup_password: "", signup_cpassword: "" });
    }
  }
  useEffect(() => {
  ...
  return (
    <form onSubmit={signup_submit} className="w-full flex flex-col items-center">
...
export default Signup_creadential
